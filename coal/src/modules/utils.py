...
def setup(bot):
    ...
    async def help(ctx):
        ...
        embed.add_field(
            name="Graveyard", value="`death`, `revive`, `obit`", inline=False
        )
    ...
    async def color(ctx, hexcode):
        ...
        embed = discord.Embed(
            color=int(hexcode.replace("#", "0x"), 0),
    ...
    async def whois(ctx, user: discord.Member | None = None):
        if user is None:
            user = ctx.author  # type: ignore[assignment]  # always Member in guild context
        ...
        embed.add_field(
            name="Roles", value=", ".join([role.name for role in user.roles])
        )
    ...
    async def snipe(ctx):
        ...
        if sniped_data:
            ...
            embed = discord.Embed(
                description=content, color=discord.Color.purple(), timestamp=timestamp
            )
            embed.set_author(
                name=str(author), icon_url=author.avatar.url if author.avatar else None
            )
    ...
    async def schedule_send(
        ctx, channel: discord.TextChannel, date: str, time: str, *, message: str
    ):
        ...
        try:
            ...
            async def send_scheduled_message():
                ...
                try:
                    ...
                    await ctx.send(
                        f"Scheduled message has been sent to {channel.mention}"
                    )
                except:
